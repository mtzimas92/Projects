...
def initialize_parameters(dx, dt, a_coeff, b_coeff, c_coeff):
    """
    ...
    Args:
        ...
        a_coeff (float): Coefficient a.
        b_coeff (float): Coefficient b.
        c_coeff (float): Coefficient c.
        
    Returns:
        tuple: Coefficients C1, C2, C3, C4, spatial domain x, temporal domain t, 
               number of spatial steps Nx, number of temporal steps Nt, and index indx.
    """
    C1 = -(a_coeff / (2 * dt))
    C2 = ((c_coeff**2) * dt) / (dx**2)
    C3 = b_coeff / 2
    C4 = a_coeff / 2
    ...
    return C1, C2, C3, C4, x, t, Nx, Nt, indx
